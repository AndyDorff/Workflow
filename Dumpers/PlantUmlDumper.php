...
namespace Modules\Workflow\Dumpers;
...
class PlantUmlDumper implements DumperInterface
{
    ...
    public function dump(Definition $definition, Marking $marking = null, array $options = []): string
    {
        ...
        if ($this->isWorkflowTransitionType()) {
            foreach ($definition->getTransitions() as $transition) {
                $title = $workflowMetadata->getMetadata('title', $transition) ?? $transition->getName();
                $transitionEscaped = $this->escape($title);
                $backgroundColor = $workflowMetadata->getMetadata('bg_color', $transition);
                if (null !== $backgroundColor) {
                    $transitionEscaped .= ' #'.$this->getColorId($backgroundColor);
                }
        ...
        foreach ($definition->getTransitions() as $transition) {
            ...
            foreach ($transition->getFroms() as $from) {
                $title = $workflowMetadata->getMetadata('title', $from) ?? $from;
                $fromEscaped = $this->escape($title);
                foreach ($transition->getTos() as $to) {
                    $title = $workflowMetadata->getMetadata('title', $to) ?? $to;
                    $toEscaped = $this->escape($title);
    ...
    private function getState(string $place, Definition $definition, Marking $marking = null): string
    {
        ...
        $title = $workflowMetadata->getMetadata('title', $place) ?? $place;
        $placeEscaped = $this->escape($title);
